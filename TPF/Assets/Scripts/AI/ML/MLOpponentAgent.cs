    ...
    public class MLOpponentAgent : Agent
    {
        ...
        public override void OnEpisodeBegin()
        {
            ...
            if (isTraining)
            {
                ...
                _keysFound = 0;
                _sensor.RayLayerMask &= ~(1 << LayerMask.NameToLayer(Layer.Door.ToString()));
                _sceneInitializer.Restart(trainingScene);
                gameManager.InitializeScene();
            }
        }
        ...
        private void OnTriggerEnter(Collider other)
        {
            ...
            if (other.gameObject.layer == LayerMask.NameToLayer(Layer.Door.ToString()))
            {
                if (gameObject.GetComponent<Player>().KeysCollected >= 1 && gameManager.isDoor(other.gameObject) ||
